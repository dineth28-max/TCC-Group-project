...
export default function TimetableBuilder() {
  ...
  async function handleCreate(e) {
    ...
    if (!selectedClass?.teacherUserId) {
      setError("This class has no teacher assigned. Assign a teacher to the class before scheduling a slot.");
      return;
    }
    try {
        ...
        <div className="col-span-2 bg-white rounded-lg shadow-sm border border-violet-100 overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody>
              {slots.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-4 py-4 text-slate-500 text-xs">
                ...
                slots
                  ...
                  .map((s) => (
                    <tr key={s.id} className="border-t border-violet-100">
                      ...
                      <td className="px-4 py-2 text-xs text-slate-500">{s.teacherName ?? "Unassigned"}</td>
            ...
            <div>
              ...
              <p
                className={`w-full border rounded px-3 py-2 text-sm ${
                  selectedClass?.teacherUserId ? "border-slate-200 bg-slate-50 text-slate-700" : "border-red-300 bg-red-50 text-red-600"
                }`}
              >
